...
pub fn poststratify_impl(
    ...
    for (i, &ctrl) in control.iter().enumerate() {
        if ctrl < 0.0 {
            return Err(WeightingError::InvalidInput(format!(
                "Control total for stratum {} is negative: {}",
                i, ctrl
            )));
        }
    }
...
pub fn poststratify_factor(
    ...
    for (i, &f) in factor.iter().enumerate() {
        if f < 0.0 {
            return Err(WeightingError::InvalidInput(format!(
                "Factor for stratum {} is negative: {}",
                i, f
            )));
        }
    }
    ...
    for r in 0..n_reps {
        ...
        let normalized = normalize_impl(wgt_2d.view(), Some(&by_arr.to_owned()), Some(&ctrl_col))?;
