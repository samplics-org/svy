...
static readstat_error_t
readstat_write_row_default_callback(void *writer_ctx, void *bytes, size_t len) {
...
readstat_writer_t *readstat_writer_init() {
  ...
  writer->variables =
      calloc(VARIABLES_INITIAL_CAPACITY, sizeof(readstat_variable_t *));
  writer->variables_capacity = VARIABLES_INITIAL_CAPACITY;

  writer->label_sets =
      calloc(LABEL_SETS_INITIAL_CAPACITY, sizeof(readstat_label_set_t *));
  ...
  writer->string_refs =
      calloc(STRING_REFS_INITIAL_CAPACITY, sizeof(readstat_string_ref_t *));
...
static void readstat_label_set_free(readstat_label_set_t *label_set) {
  ...
  for (i = 0; i < label_set->value_labels_count; i++) {
    readstat_value_label_t *value_label =
        readstat_get_value_label(label_set, i);
...
static readstat_value_label_t *
readstat_add_value_label(readstat_label_set_t *label_set, const char *label) {
  if (label_set->value_labels_count == label_set->value_labels_capacity) {
    label_set->value_labels_capacity *= 2;
    label_set->value_labels =
        realloc(label_set->value_labels, label_set->value_labels_capacity *
                                             sizeof(readstat_value_label_t));
  }
  readstat_value_label_t *new_value_label =
      &label_set->value_labels[label_set->value_labels_count++];
...
readstat_error_t
readstat_validate_variable(readstat_writer_t *writer,
...
static readstat_error_t readstat_begin_writing_data(readstat_writer_t *writer) {
  ...
  for (i = 0; i < writer->variables_count; i++) {
    ...
    variable->storage_width =
        writer->callbacks.variable_width(variable->type, variable->user_width);
  ...
  if (writer->callbacks.variable_ok) {
    for (i = 0; i < writer->variables_count; i++) {
      retval =
          readstat_validate_variable(writer, readstat_get_variable(writer, i));
      if (retval != READSTAT_OK)
        goto cleanup;
    }
  }
...
readstat_error_t readstat_set_data_writer(readstat_writer_t *writer,
                                          readstat_data_writer data_writer) {
  writer->data_writer = data_writer;
  return READSTAT_OK;
}

readstat_error_t readstat_write_bytes(readstat_writer_t *writer,
...
readstat_error_t readstat_write_bytes_as_lines(readstat_writer_t *writer,
  ...
  while (bytes_written < len) {
    ssize_t bytes_left_in_line =
        line_len - (writer->bytes_written % (line_len + line_sep_len));
    if (len - bytes_written < bytes_left_in_line) {
      retval = readstat_write_bytes(
          writer, ((const char *)bytes) + bytes_written, len - bytes_written);
      bytes_written = len;
    } else {
      retval = readstat_write_bytes(
          writer, ((const char *)bytes) + bytes_written, bytes_left_in_line);
    ...
    if (writer->bytes_written % (line_len + line_sep_len) == line_len) {
      if ((retval = readstat_write_bytes(writer, line_sep, line_sep_len)) !=
          READSTAT_OK)
        break;
    }
  }
...
readstat_error_t readstat_write_line_padding(readstat_writer_t *writer,
                                             char pad, size_t line_len,
                                             const char *line_sep) {
  size_t line_sep_len = strlen(line_sep);
  ...
  if ((error = readstat_write_bytes(writer, bytes, bytes_left_in_line)) !=
      READSTAT_OK)
    goto cleanup;

  if ((error = readstat_write_bytes(writer, line_sep, line_sep_len)) !=
      READSTAT_OK)
    goto cleanup;
...
readstat_error_t readstat_write_string(readstat_writer_t *writer,
                                       const char *bytes) {
  return readstat_write_bytes(writer, bytes, strlen(bytes));
}

static readstat_error_t readstat_write_repeated_byte(readstat_writer_t *writer,
                                                     char byte, size_t len) {
  if (len == 0)
    return READSTAT_OK;
...
readstat_error_t readstat_write_space_padded_string(readstat_writer_t *writer,
                                                    const char *string,
                                                    size_t max_len) {
  readstat_error_t retval = READSTAT_OK;
...
readstat_label_set_t *readstat_add_label_set(readstat_writer_t *writer,
  ...
  new_label_set->value_labels =
      calloc(VALUE_LABELS_INITIAL_CAPACITY, sizeof(readstat_value_label_t));
  new_label_set->value_labels_capacity = VALUE_LABELS_INITIAL_CAPACITY;

  new_label_set->variables = calloc(LABEL_SET_VARIABLES_INITIAL_CAPACITY,
                                    sizeof(readstat_variable_t *));
  new_label_set->variables_capacity = LABEL_SET_VARIABLES_INITIAL_CAPACITY;

  return new_label_set;
}

readstat_label_set_t *readstat_get_label_set(readstat_writer_t *writer,
                                             int index) {
...
void readstat_sort_label_set(readstat_label_set_t *label_set,
                             int (*compare)(const readstat_value_label_t *,
                                            const readstat_value_label_t *)) {
  qsort(label_set->value_labels, label_set->value_labels_count,
        sizeof(readstat_value_label_t),
        (int (*)(const void *, const void *))compare);
}

readstat_value_label_t *
readstat_get_value_label(readstat_label_set_t *label_set, int index) {
...
readstat_variable_t *
readstat_get_label_set_variable(readstat_label_set_t *label_set, int index) {
...
void readstat_label_double_value(readstat_label_set_t *label_set, double value,
                                 const char *label) {
  readstat_value_label_t *new_value_label =
      readstat_add_value_label(label_set, label);
  new_value_label->double_key = value;
  new_value_label->int32_key = value;
}

void readstat_label_int32_value(readstat_label_set_t *label_set, int32_t value,
                                const char *label) {
  readstat_value_label_t *new_value_label =
      readstat_add_value_label(label_set, label);
  new_value_label->double_key = value;
...
void readstat_label_tagged_value(readstat_label_set_t *label_set, char tag,
                                 const char *label) {
  readstat_value_label_t *new_value_label =
      readstat_add_value_label(label_set, label);
...
readstat_variable_t *readstat_add_variable(readstat_writer_t *writer,
  ...
  if (readstat_variable_get_type_class(new_variable) ==
...
void readstat_add_note(readstat_writer_t *writer, const char *note) {
  if (writer->notes_count == writer->notes_capacity) {
    ...
    writer->notes =
        realloc(writer->notes, writer->notes_capacity * sizeof(const char *));
  }
...
void readstat_variable_set_format(readstat_variable_t *variable,
                                  const char *format) {
  if (format) {
...
void readstat_variable_set_alignment(readstat_variable_t *variable,
                                     readstat_alignment_t alignment) {
  variable->alignment = alignment;
}

void readstat_variable_set_display_width(readstat_variable_t *variable,
                                         int display_width) {
  variable->display_width = display_width;
}

void readstat_variable_set_label_set(readstat_variable_t *variable,
  ...
  if (label_set) {
    if (label_set->variables_count == label_set->variables_capacity) {
      ...
      label_set->variables =
          realloc(label_set->variables, label_set->variables_capacity *
                                            sizeof(readstat_variable_t *));
    }
    ((readstat_variable_t **)
         label_set->variables)[label_set->variables_count++] = variable;
  }
}

readstat_variable_t *readstat_get_variable(readstat_writer_t *writer,
                                           int index) {
...
readstat_string_ref_t *readstat_get_string_ref(readstat_writer_t *writer,
                                               int index) {
...
readstat_error_t readstat_writer_set_file_label(readstat_writer_t *writer,
                                                const char *file_label) {
  snprintf(writer->file_label, sizeof(writer->file_label), "%s", file_label);
  return READSTAT_OK;
}

readstat_error_t readstat_writer_set_file_timestamp(readstat_writer_t *writer,
                                                    time_t timestamp) {
  writer->timestamp = timestamp;
  return READSTAT_OK;
}

readstat_error_t readstat_writer_set_table_name(readstat_writer_t *writer,
...
readstat_error_t
readstat_writer_set_fweight_variable(readstat_writer_t *writer,
                                     const readstat_variable_t *variable) {
...
readstat_error_t
readstat_writer_set_file_format_version(readstat_writer_t *writer,
                                        uint8_t version) {
...
readstat_error_t
readstat_writer_set_file_format_is_64bit(readstat_writer_t *writer,
                                         int is_64bit) {
...
readstat_error_t
readstat_writer_set_compression(readstat_writer_t *writer,
...
readstat_error_t
readstat_writer_set_error_handler(readstat_writer_t *writer,
                                  readstat_error_handler error_handler) {
  writer->error_handler = error_handler;
  return READSTAT_OK;
}

readstat_error_t readstat_begin_writing_file(readstat_writer_t *writer,
...
readstat_error_t readstat_insert_int8_value(readstat_writer_t *writer,
                                            const readstat_variable_t *variable,
                                            int8_t value) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;
  ...
  return writer->callbacks.write_int8(&writer->row[variable->offset], variable,
                                      value);
}

readstat_error_t
readstat_insert_int16_value(readstat_writer_t *writer,
                            const readstat_variable_t *variable,
                            int16_t value) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;
  ...
  return writer->callbacks.write_int16(&writer->row[variable->offset], variable,
                                       value);
}

readstat_error_t
readstat_insert_int32_value(readstat_writer_t *writer,
                            const readstat_variable_t *variable,
                            int32_t value) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;
  ...
  return writer->callbacks.write_int32(&writer->row[variable->offset], variable,
                                       value);
}

readstat_error_t
readstat_insert_float_value(readstat_writer_t *writer,
                            const readstat_variable_t *variable, float value) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;
  if (variable->type != READSTAT_TYPE_FLOAT)
    return READSTAT_ERROR_VALUE_TYPE_MISMATCH;

  return writer->callbacks.write_float(&writer->row[variable->offset], variable,
                                       value);
}

readstat_error_t
readstat_insert_double_value(readstat_writer_t *writer,
                             const readstat_variable_t *variable,
                             double value) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;
  ...
  return writer->callbacks.write_double(&writer->row[variable->offset],
                                        variable, value);
}

readstat_error_t
readstat_insert_string_value(readstat_writer_t *writer,
                             const readstat_variable_t *variable,
                             const char *value) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;
  ...
  return writer->callbacks.write_string(&writer->row[variable->offset],
                                        variable, value);
}
...
readstat_error_t readstat_insert_string_ref(readstat_writer_t *writer,
  ...
  return writer->callbacks.write_string_ref(&writer->row[variable->offset],
                                            variable, ref);
}

readstat_error_t
readstat_insert_missing_value(readstat_writer_t *writer,
                              const readstat_variable_t *variable) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;

  if (variable->type == READSTAT_TYPE_STRING) {
    return writer->callbacks.write_missing_string(
        &writer->row[variable->offset], variable);
  }
  if (variable->type == READSTAT_TYPE_STRING_REF) {
    return readstat_insert_string_ref(writer, variable, NULL);
  }

  return writer->callbacks.write_missing_number(&writer->row[variable->offset],
                                                variable);
}

readstat_error_t readstat_insert_tagged_missing_value(
    readstat_writer_t *writer, const readstat_variable_t *variable, char tag) {
  if (!writer->initialized)
    return READSTAT_ERROR_WRITER_NOT_INITIALIZED;
  if (!writer->callbacks.write_missing_tagged) {
    /* Write out a missing number but return an error */
    writer->callbacks.write_missing_number(&writer->row[variable->offset],
                                           variable);
    return READSTAT_ERROR_TAGGED_VALUES_NOT_SUPPORTED;
  }

  return writer->callbacks.write_missing_tagged(&writer->row[variable->offset],
                                                variable, tag);
}
...
readstat_error_t readstat_end_row(readstat_writer_t *writer) {
  ...
  readstat_error_t error =
      writer->callbacks.write_row(writer, writer->row, writer->row_len);
...
readstat_error_t readstat_end_writing(readstat_writer_t *writer) {
  ...
  for (i = 1; i < writer->string_refs_count; i++) {
    if (readstat_compare_string_refs(&writer->string_refs[i - 1],
                                     &writer->string_refs[i]) > 0) {
