...
`polars-svy` implements survey variance estimation using Taylor series linearization:
...
Compared to pure Python/NumPy implementations, `polars-svy` provides:
...
`polars-svy` is designed to be used as a computational backend for the `svy` library:
