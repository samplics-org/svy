...
- **`singleton.scale()` point estimates.** The full-sample pass read the raw data frame instead of the prepared one. An unrelated Int8/Int16/UInt8/UInt16 column anywhere in the frame raised `ComputeError: cannot create series from Int8`; `by=` raised a missing-column error; `ratio()` with an integer denominator raised a dtype error; and `where=` silently returned the whole-sample estimate instead of the domain estimate. That pass now reuses the same preparation as the variance pass, keeping singleton strata in. Standard errors are unchanged.
